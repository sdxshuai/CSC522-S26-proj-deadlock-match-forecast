...
def fetch_hero_stats(out_dir: Path) -> None:
    print("Fetching /v1/analytics/hero-stats ...")
    ...
    print(f"  Saved {len(data)} hero records → {out_path}")


def fetch_counter_stats(out_dir: Path) -> None:
    print("Fetching /v1/analytics/hero-counter-stats ...")
    ...
    out_path.write_text(json.dumps(data, indent=2))
    print(f"  Saved {len(data)} matchup records → {out_path}")


def fetch_synergy_stats(out_dir: Path) -> None:
    print("Fetching /v1/analytics/hero-synergy-stats ...")
    data = fetch_json(
    ...
    out_path.write_text(json.dumps(data, indent=2))
    print(f"  Saved {len(data)} synergy records → {out_path}")
...
def main() -> None:
    ...
    print("\nDone. All hero stats written to", out_dir)
